              ...
              <div className="grid grid-cols-2 md:gap-5 gap-3 mt-6 w-full md:max-w-md">
                {listOfLinks.slice(0, 3).map((item, index) =>
                  item.links.map((link, linkIndex) => (
                    <div
                      className={`md:px-5 px-3 rounded-2xl shadow-inner bg-white/5 text-white flex  ${index === 2 ? "col-span-2 flex-row items-center justify-center gap-4 py-6" : "flex-col py-2.5 gap-2.5 "}`}
                      key={linkIndex}
                    >
                      <div>
                ...
                {listOfLinks.slice(3).map((item, index) => (
                  <div className="space-y-4" key={index}>
                    {item.title && <h3 className="text-white md:text-md text-sm font-medium">{item.title}</h3>}
