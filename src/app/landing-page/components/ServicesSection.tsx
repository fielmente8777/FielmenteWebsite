...
const ServicesSection: React.FC<ServicesSectionProps> = ({
  ...
  return (
    <SectionWithContainer sectionClassName="relative after:absolute after:inset-2 after:rounded-2xl before:bg-background-black1 before:content-[''] before:inset-0 before:absolute after:bg-[#F9FAFB] before:z-[-2] after:z-[-1]">
      ...
      />
      <div className="flex flex-wrap md:max-w-[50.4rem] md:items-center max-md:justify-center w-full md:mt-8 mt-4 mx-auto md:gap-4 gap-2">
      ...
      <div className="lg:grid hidden grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-[1.9rem] gap-x-7  mt-10">
        {services.map((service, index) => (
          <ServiceCard key={index} {...service} />
        ))}
      </div>
      <ServiceSlider services={services} />
      <div className="border border-main-border bg-gradient-to-r from-30% from-[#Eff4ff] to-70% to-[#f7f7f7] md:py-7.5 py-5 md:px-13.5 px-3 rounded-2xl md:mt-14 mt-6 flex max-md:flex-col items-center gap-5 justify-between">
...
export const PointsCards = ({
  ...
  return (
    <div className="w-fit flex items-center gap-4 border border-main-border rounded-full lg:px-5 px-4 lg:py-3 py-2">
      {icon && <Foo />}
      <p
        className="text-secondary2 lg:text-lg md:text-sm text-[9px]"
