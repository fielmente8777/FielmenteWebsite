  ...
  return (
    <Section>
      ...
      <div className="md:max-w-2xl w-full max-md:px-4 mx-auto mt-12 text-center space-y-4 poppins">
        <p className="text-gray-primary md:text-lg">{description}</p>
        {pathName === "/landing-page/" ? (
          <Link
            href={contacts.WhatsAppCta}
            onClick={() => setIsOpenPopupForm(true)}
            className="inline-block bg-orange-primary font-medium text-white text-lg/[24px] py-4 px-6 rounded-md"
          >
            {button?.label}
          </Link>
        ) : (
