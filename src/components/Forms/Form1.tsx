...
const Form1 = ({ bgWhite = false }: { bgWhite?: boolean }) => {
  ...
  return (
    <form
      ...
      className={`flex flex-col gap-4 max-md:px-4 p-6 max-md:mt-6 text-base rounded-[18px] lg:max-w-[24.875rem] w-full ${bgWhite && bgWhite ? "bg-white" : "bg-primary"}`}
    >
      <h2
        className={`text-xl md:text-2xl font-semibold ${!bgWhite && !bgWhite ? "text-white" : "text-primary"} `}
      >
